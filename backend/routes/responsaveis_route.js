...
// Contar responsáveis cadastrados
router.get('/count', (req, res) => {
  const sql = 'SELECT COUNT(*) AS total FROM responsavel';
  connection.query(sql, (err, results) => {
    if (err) {
      console.error("Erro ao contar responsáveis:", err);
      return res.status(500).send('Erro ao contar responsáveis');
    }
...
router.post('/', (req, res) => {
  ...
  const sql = `
    ...
    (nome, sobrenome, telefone, email, cpf_responsavel, data_nascimento, sexo, parentesco, senha, estado_civil, endereco_id_endereco, id_crianca)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `
  connection.query(sql, [nome, sobrenome, telefone, email, cpf_responsavel, data_nascimento, sexo, parentesco, senha, estado_civil, endereco_id_endereco, id_crianca], (err, result) => {
...
router.put('/:id', (req, res) => {
  ...
  const sql = `
    UPDATE responsavel SET 
      nome = ?, sobrenome = ?, telefone = ?, email = ?, cpf_responsavel = ?, data_nascimento = ?, 
      sexo = ?, parentesco = ?, senha = ?, estado_civil = ?, endereco_id_endereco = ?, id_crianca = ?
    WHERE id = ?
  `
  connection.query(sql, [nome, sobrenome, telefone, email, cpf_responsavel, data_nascimento, sexo, parentesco, senha, estado_civil, endereco_id_endereco, id_crianca, id], (err, result) => {
